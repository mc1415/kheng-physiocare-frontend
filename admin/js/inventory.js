...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function fetchAndRenderProducts() { if (!productTableBody) return; productTableBody.innerHTML = `<tr><td colspan="7">Loading...</td></tr>`; const result = await fetchApi('/api/products'); if(result && result.success) { renderProductTable(result.data); } else { productTableBody.innerHTML = `<tr><td colspan="7" style="text-align:center; color: var(--red-accent);">Failed to load products.</td></tr>`; } }
    ...
    async function openModalForEdit(productId) { modalTitle.textContent = 'Edit Product'; saveBtn.textContent = 'Save Changes'; productForm.reset(); productIdField.value = productId; const result = await fetchApi(`/api/products/${productId}`); if (result && result.success) { const product = result.data; document.getElementById('product-name').value = product.name; document.getElementById('product-sku').value = product.sku; document.getElementById('product-category').value = product.category; document.getElementById('product-stock').value = product.stock_level; document.getElementById('product-price').value = product.unit_price; addProductModal.style.display = 'flex'; } }
