...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function fetchAndRenderInvoices() { if (!invoiceHistoryBody) return; invoiceHistoryBody.innerHTML = `<tr><td colspan="6" style="text-align: center;">Loading...</td></tr>`; const result = await fetchApi('/api/invoices'); if (result && result.success) { allInvoices = result.data; renderInvoiceTable(allInvoices); } else { invoiceHistoryBody.innerHTML = `<tr><td colspan="6" style="text-align: center; color: var(--red-accent);">Failed to fetch invoice history.</td></tr>`; } }
    ...
    async function populatePatientDropdown() { const patientSelect = document.getElementById('invoice-patient'); if(!patientSelect) return; const result = await fetchApi('/api/patients'); if(result && result.success) { patientSelect.innerHTML = '<option value="">-- Select Patient --</option>'; result.data.forEach(patient => { const option = document.createElement('option'); option.value = patient.raw_id; option.textContent = `${patient.fullName} (${patient.display_id})`; patientSelect.appendChild(option); }); } else { patientSelect.innerHTML = '<option value="">Could not load patients</option>'; } }
    async function getBillableItems() { try { const result = await fetchApi('/api/products'); const inventoryProducts = (result && result.success) ? result.data.map(prod => ({ id: prod.id, name: prod.name, price: parseFloat(prod.unit_price).toFixed(2), isService: false })) : []; return [...professionalServices, ...inventoryProducts]; } catch (error) { console.error("Error fetching billable items:", error); return professionalServices; } }
    ...
    async function openModalForEdit(invoiceId) { const modalTitle = createInvoiceModal.querySelector('h2'); const saveButton = createInvoiceModal.querySelector('button[type="submit"]'); invoiceForm.reset(); invoiceItemsContainer.innerHTML = ''; modalTitle.textContent = `Edit Invoice #${invoiceId.toString().padStart(5, '0')}`; saveButton.textContent = 'Save Changes'; if (!document.getElementById('invoiceIdField')) { invoiceForm.insertAdjacentHTML('afterbegin', `<input type="hidden" id="invoiceIdField" name="invoiceId">`); } document.getElementById('invoiceIdField').value = invoiceId; await populatePatientDropdown(); const result = await fetchApi(`/api/invoices/${invoiceId}`); if(result && result.success) { const inv = result.data; document.getElementById('invoice-patient').value = inv.patient_id; document.getElementById('invoice-status').value = inv.status; if (inv.items && inv.items.length > 0) { for (const item of inv.items) { await addInvoiceItemRow(item); } } else { await addInvoiceItemRow(); } } createInvoiceModal.style.display = 'flex'; }
    ...
    if(invoiceHistoryBody) { invoiceHistoryBody.addEventListener('click', async (event) => { const targetButton = event.target.closest('button.btn-action'); if (!targetButton) return; const invoiceId = targetButton.dataset.invoiceId; if (targetButton.classList.contains('print')) { const result = await fetchApi(`/api/invoices/${invoiceId}`); if (result && result.success) { const invData = result.data; const receiptData = { patientName: invData.patients.full_name, patientAge: invData.patients.date_of_birth ? new Date().getFullYear() - new Date(invData.patients.date_of_birth).getFullYear() : 'N/A', billDate: new Date(invData.created_at).toISOString().split('T')[0], items: invData.items.map(item => ({ service: item.service_name, qty: item.quantity, price: item.unit_price, disc: 0 })) }; localStorage.setItem('currentInvoiceData', JSON.stringify(receiptData)); window.open('../kheng-physiocare-receipt.html', '_blank'); } } else if (targetButton.classList.contains('pay')) { const toast = Toastify({ text: "<strong>Mark this invoice as paid?</strong>", escapeMarkup: !1, duration: -1, close: !0, gravity: "center", position: "center", style: { background: "#1e293b", border: "1px solid var(--primary-accent)" } }).showToast(); const toastEl = toast.toastElement; const buttonContainer = document.createElement('div'); buttonContainer.style.cssText = "margin-top: 15px; text-align: right;"; const yesButton = document.createElement('button'); yesButton.innerText = "Yes, Mark as Paid"; yesButton.className = "btn-primary-action"; yesButton.style.marginRight = "10px"; const noButton = document.createElement('button'); noButton.innerText = "Cancel"; noButton.className = "btn-secondary-action"; buttonContainer.append(noButton, yesButton); toastEl.appendChild(buttonContainer); yesButton.onclick = async function() { toast.hideToast(); const result = await fetchApi(`/api/invoices/${invoiceId}/pay`, { method: 'PATCH' }); if (result && result.success) { Toastify({...toastConfig, text: "Invoice marked as paid!", style: { background: "var(--primary-accent)" }}).showToast(); fetchAndRenderInvoices(); } }; noButton.onclick = function() { toast.hideToast(); } } else if (targetButton.classList.contains('edit')) { openModalForEdit(invoiceId); } else if (targetButton.classList.contains('delete')) { const toast = Toastify({ text: "<strong>Delete this invoice?</strong><br>This action is permanent.", escapeMarkup: !1, duration: -1, close: !0, gravity: "center", position: "center", style: { background: "#1e293b", border: "1px solid var(--red-accent)" } }).showToast(); const toastEl = toast.toastElement; const buttonContainer = document.createElement('div'); buttonContainer.style.cssText = "margin-top: 15px; text-align: right;"; const yesButton = document.createElement('button'); yesButton.innerText = "Yes, Delete"; yesButton.className = "btn-danger"; yesButton.style.marginRight = "10px"; const noButton = document.createElement('button'); noButton.innerText = "Cancel"; noButton.className = "btn-secondary-action"; buttonContainer.append(noButton, yesButton); toastEl.appendChild(buttonContainer); yesButton.onclick = async function() { toast.hideToast(); const result = await fetchApi(`/api/invoices/${invoiceId}`, { method: 'DELETE' }); if (result && result.success) { Toastify({...toastConfig, text: "Invoice deleted.", style: { background: "var(--red-accent)" }}).showToast(); fetchAndRenderInvoices(); } }; noButton.onclick = function() { toast.hideToast(); } } }); }
    if(invoiceForm) { invoiceForm.addEventListener('submit', async function(event) { event.preventDefault(); const formData = new FormData(invoiceForm); const invoiceId = formData.get('invoiceId'); const isEditing = !!invoiceId; const items = []; const inventoryUpdates = []; const itemRows = invoiceItemsContainer.querySelectorAll('.invoice-item-row'); itemRows.forEach((row, index) => { const serviceSelect = row.querySelector('[name="service"]'); const selectedOption = serviceSelect.options[serviceSelect.selectedIndex]; const productId = selectedOption.dataset.productId; const quantity = formData.getAll('quantity')[index]; items.push({ service: formData.getAll('service')[index], quantity: quantity, price: formData.getAll('price')[index] }); if (productId) { inventoryUpdates.push({ id: productId, quantitySold: parseInt(quantity) }); } }); const invoiceData = { patientId: formData.get('patientId'), status: formData.get('status'), items, inventoryUpdates }; const url = isEditing ? `/api/invoices/${invoiceId}` : '/api/invoices'; const method = isEditing ? 'PATCH' : 'POST'; const result = await fetchApi(url, { method: method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(invoiceData) }); if (result) { const successMessage = isEditing ? "Invoice updated successfully!" : "Invoice created successfully!"; const toastColor = isEditing ? "var(--primary-accent)" : "var(--green-accent)"; Toastify({...toastConfig, text: successMessage, style: { background: toastColor }}).showToast(); createInvoiceModal.style.display = 'none'; setTimeout(() => fetchAndRenderInvoices(), 500); } }); }
